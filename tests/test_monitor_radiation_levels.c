...
Test(monitor_radiation_levels, no_marker_on_small_positive) {
    redirect_all_stdio();
    simulate_input("5");
...
Test(monitor_radiation_levels, no_marker_on_zero) {
    redirect_all_stdio();
    simulate_input("0");
    monitor_radiation_levels();
    char *out = get_stdout_content();
...
Test(monitor_radiation_levels, no_marker_on_negative) {
    redirect_all_stdio();
    simulate_input("-42");
    monitor_radiation_levels();
    char *out = get_stdout_content();
...
Test(monitor_radiation_levels, no_marker_on_large_number) {
    redirect_all_stdio();
...
Test(monitor_radiation_levels, no_marker_on_random_number) {
    redirect_all_stdio();
